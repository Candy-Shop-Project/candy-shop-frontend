...
function Cart() {
  ...
  useEffect(() => {
    const items = JSON.parse(localStorage.getItem("cart_keys") || "[]"); // get items from localStorage
    dispatch(setCartCount(items.length)); // set redux state to current count(in order to be in sync with localStorage if user refreshes page)
  }, [dispatch]); // dispatch as dependency array, in order for this component to run when there are dispatch action
